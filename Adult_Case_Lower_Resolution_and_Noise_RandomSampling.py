...
from modulusDL.pdes.FSI.length1_velscale_8.modified_temporal_navier_stokes_first_time_lagrange import TemporalNavierStokes_1st_time_lagrange, wall_shear_stress
from modulusDL.pdes.FSI.length1_velscale_8.modified_temporal_navier_stokes_central import TemporalNavierStokes_2nd_time_point
from modulusDL.pdes.FSI.length1_velscale_8.modified_temporal_navier_stokes_2nd_time_lagrange import TemporalNavierStokes_3rd_time_point_lagrange
...
class vel_threshold(torch.nn.Module):
    def __init__(self):
        super().__init__()
        
    def forward(self,x):#1:r_x,2:r_y,3:r_z 4:u, 5:v, 6:w
        
        # Define the equation as the difference between velocity magnitude and threshold, only if velocity magnitude is greater than threshold
        threshold = 1.0 #0.1 normalised by 0.1 vel scale
        shortfall = torch.abs(x[...,0:1]) - threshold
        
        vel_constraint = torch.nn.functional.softplus(shortfall)
...
def run(cfg: ModulusConfig) -> None:
    ...
    rho = quantity(1056, "kg/m^3")
    
    center_hardcode = (0.046012, 0.019668, -0.13804)
    
    length_scale_amp = 0.1  # turn off scaling for now
    vel_scale_amp = 0.8
    
    # distance_to_transducer = 0.1 #this is in meters
    distance_to_transducer = (-0.0626811,0.0380503,0.033536)
    transducer_center_hardcode = (0.046012, 0.019668, -0.13804)
    ...
    point_path = to_absolute_path("./FSI_model")
    
    interior_mesh_pre = Tessellation.from_stl(
        point_path + "/3_point_training_with_105/105/capped_cut_down_105.stl", airtight=True
    )
    
    interior_mesh_2nd_time = Tessellation.from_stl(
        point_path + "/3_point_training_with_105/109_cut_down/CUT_DOWN_GEOMETRY_capped.stl", airtight=True
    )
    interior_mesh_3rd_time = Tessellation.from_stl(
        point_path + "/3_point_training_with_105/113_cut_down/capped_113.stl", airtight=True
    )
    ...
    def normalize_invar_vel_pre(invar, center, scale, vel_scale, dims=2):
    ...
    def normalize_invar_vel(invar, center, scale, vel_scale, dims=2):
    ...
    def normalize_invar_vel_pre_with_pressure(invar, center, scale, vel_scale, pres_scale, dims=2):
    ...
    # interior_intersection = interior_mesh_pre & interior_mesh_post
    ...
    wss = wall_shear_stress(nu=nd.ndim(nu), rho=nd.ndim(rho), dim=3)
    ######################     DISTANCE NET ##################
    ...
    US_doppler_pre = CustomModuleArch(
        ...
        output_keys=[Key("us_mag_noisy_0")],
    ...
    nodes = (
            ...
            + Scaler(
                ["u_0","v_0", "w_0", "p_0", "u_1","v_1", "w_1", "p_1", "u", "v","w","p","partial_ux", "partial_uy", "partial_uz", "partial_vx", "partial_vy", "partial_vz", "partial_wx", "partial_wy", "partial_wz", "partial_ux_1", "partial_uy_1", "partial_uz_1", "partial_vx_1", "partial_vy_1", "partial_vz_1", "partial_wx_1", "partial_wy_1", "partial_wz_1", "partial_ux_0", "partial_uy_0", "partial_uz_0", "partial_vx_0", "partial_vy_0", "partial_vz_0", "partial_wx_0", "partial_wy_0", "partial_wz_0"],
                ["u_0_scaled", "v_0_scaled", "w_0_scaled", "p_0_scaled", "u_1_scaled", "v_1_scaled", "w_1_scaled", "p_1_scaled", "u_scaled", "v_scaled", "w_scaled", "p_scaled","partial_ux_scaled", "partial_uy_scaled", "partial_uz_scaled", "partial_vx_scaled", "partial_vy_scaled", "partial_vz_scaled", "partial_wx_scaled", "partial_wy_scaled", "partial_wz_scaled", "partial_ux_1_scaled", "partial_uy_1_scaled", "partial_uz_1_scaled", "partial_vx_1_scaled", "partial_vy_1_scaled", "partial_vz_1_scaled", "partial_wx_1_scaled", "partial_wy_1_scaled", "partial_wz_1_scaled", "partial_ux_0_scaled", "partial_uy_0_scaled", "partial_uz_0_scaled", "partial_vx_0_scaled", "partial_vy_0_scaled", "partial_vz_0_scaled", "partial_wx_0_scaled", "partial_wy_0_scaled", "partial_wz_0_scaled"],
                ["m/s", "m/s","m/s", "m^2/s^2","m/s", "m/s","m/s", "m^2/s^2", "m/s", "m/s","m/s", "m^2/s^2", '1/s', '1/s', '1/s', '1/s', '1/s', '1/s', '1/s', '1/s', '1/s', '1/s', '1/s', '1/s', '1/s', '1/s', '1/s', '1/s', '1/s', '1/s', '1/s', '1/s', '1/s', '1/s', '1/s', '1/s', '1/s', '1/s', '1/s'],
                nd,
            ).make_node()
            + wss.make_nodes()
    )
    ...
    inflow_var = csv_to_dict(
       to_absolute_path("./FSI_model/3_point_training_with_105/105/INTERPOLATED_VELOCITIES_10_20_inlet.csv"))
    inflow_invar = {
    ...
    inflow_outvar = normalize_invar_vel_pre_with_pressure(inflow_outvar, center, 1./length_scale_amp, (1./vel_scale_amp), (1./((vel_scale_amp**2.)*1056.)), dims=3)
    ...
    domain.add_constraint(inlet_numpy, "inlet_117")
    
    inflow_var = csv_to_dict(
       to_absolute_path("./FSI_model/3_point_training_with_105/113_cut_down/INTERPOLATED_VELOCITIES_10_20_outlet.csv"))
    inflow_invar = {
       key: value for key, value in inflow_var.items() if key in ["x", "y", "z"]
    }
    inflow_invar = normalize_invar(inflow_invar, center, 1./length_scale_amp, dims=3)
    ...
    # inflow_outvar = normalize_invar_vel(inflow_outvar, center, 1./length_scale_amp, (1./vel_scale_amp), dims=3)
    #inflow_invar = normalize_velvar(inflow_invar, velocity_scale, dims=3)
    inlet_numpy = PointwiseConstraint.from_numpy(
       nodes,
       inflow_invar,
    ...
    domain.add_constraint(inlet_numpy, "outlet_113")
    
    inflow_var = csv_to_dict(
       to_absolute_path("./FSI_model/3_point_training_with_105/105/INTERPOLATED_VELOCITIES_10_20_outlet.csv"))
    inflow_invar = {
       key: value for key, value in inflow_var.items() if key in ["x", "y", "z"]
    }
    inflow_invar = normalize_invar(inflow_invar, center, 1./length_scale_amp, dims=3)
    ...
    domain.add_constraint(inlet_numpy, "outlet_117")
    
    inflow_var = csv_to_dict(
       to_absolute_path("./FSI_model/3_point_training_with_105/109_cut_down/INTERPOLATED_VELOCITIES_10_20_outlet.csv"))
    inflow_invar = {
       key: value for key, value in inflow_var.items() if key in ["x", "y", "z"]
    }
    inflow_invar = normalize_invar(inflow_invar, center, 1./length_scale_amp, dims=3)
    ...
    # inflow_outvar = normalize_invar_vel_pre(inflow_outvar, center, 1./length_scale_amp, (1./vel_scale_amp), dims=3)
    #inflow_invar = normalize_velvar(inflow_invar, velocity_scale, dims=3)
    inlet_numpy = PointwiseConstraint.from_numpy(
       nodes,
       inflow_invar,
    ...
    domain.add_constraint(inlet_numpy, "outlet_109")
    
    
    inflow_var = csv_to_dict(
       to_absolute_path("./FSI_model/3_point_training_with_105/109_cut_down/INTERPOLATED_VELOCITIES_30_40_wall_with_interior.csv"))
    inflow_invar = {
       key: value for key, value in inflow_var.items() if key in ["x", "y", "z"]
    }
    inflow_invar = normalize_invar(inflow_invar, center, 1./length_scale_amp, dims=3)
    ...
    domain.add_constraint(wall, "no_slip_pre")
    
    inflow_var = csv_to_dict(
       to_absolute_path("./FSI_model/3_point_training_with_105/113_cut_down/INTERPOLATED_VELOCITIES_30_40_with_wall_water.csv"))
    inflow_invar = {
       key: value for key, value in inflow_var.items() if key in ["x", "y", "z"]
    }
    inflow_invar = normalize_invar(inflow_invar, center, 1./length_scale_amp, dims=3)
    ...
    domain.add_constraint(wall, "no_slip_113")
    
    inflow_var = csv_to_dict(
       to_absolute_path("./FSI_model/3_point_training_with_105/105/INTERPOLATED_VELOCITIES_30_40_wall.csv"))
    inflow_invar = {
       key: value for key, value in inflow_var.items() if key in ["x", "y", "z"]
    }
    inflow_invar = normalize_invar(inflow_invar, center, 1./length_scale_amp, dims=3)
    ...
    domain.add_constraint(wall, "no_slip_117")
    
    inflow_var = csv_to_dict(
       to_absolute_path("./FSI_model/3_point_training_with_105/109_cut_down/INTERPOLATED_VELOCITIES_10_20_inlet.csv"))
    inflow_invar = {
       key: value for key, value in inflow_var.items() if key in ["x", "y", "z"]
    }
    inflow_invar = normalize_invar(inflow_invar, center, 1./length_scale_amp, dims=3)
    ...
    inflow_outvar = normalize_invar_vel_2nd_time(inflow_outvar, center, 1./length_scale_amp, (1./vel_scale_amp), (1./((vel_scale_amp**2.)*1056.)), dims=3)
    ...
    domain.add_constraint(wall, "inlet_109")
    
    inflow_var = csv_to_dict(
       to_absolute_path("./FSI_model/3_point_training_with_105/113_cut_down/INTERPOLATED_VELOCITIES_10_20_inlet.csv"))
    inflow_invar = {
       key: value for key, value in inflow_var.items() if key in ["x", "y", "z"]
    }
    inflow_invar = normalize_invar(inflow_invar, center, 1./length_scale_amp, dims=3)
    ...
    inflow_outvar = normalize_invar_vel_3rd_time(inflow_outvar, center, 1./length_scale_amp, (1./vel_scale_amp), (1./((vel_scale_amp**2.)*1056.)), dims=3)
    ...
    domain.add_constraint(wall, "inlet_113")
    ...
    domain.add_constraint(interior, "interior_117")
    ...
    domain.add_constraint(interior, "interior_113")
    ...
    domain.add_constraint(interior, "interior_109")
    
    # inflow_var = csv_to_dict(
       # to_absolute_path("./FSI_model/3_point_training_with_105/109_cut_down/US_threshold_cut_down_interpolated_points.csv"))
    # inflow_invar = {
       # key: value for key, value in inflow_var.items() if key in ["x", "y", "z"]
    # }
    # inflow_invar = normalize_invar(inflow_invar, center, 1./length_scale_amp, dims=3)
    ...
    # doppler_numpy = PointwiseConstraint.from_numpy(
       ...
       # batch_size=10000,
    # )
    # domain.add_constraint(doppler_numpy, "color_doppler_mag_pre")
    
    inflow_var = csv_to_dict(
       to_absolute_path("./FSI_model/3_point_training_with_105/113_cut_down/sampled_DOPPLER_SIGNAL_cut_down_interpolated_points_with_noise_threshold.csv"))
    inflow_invar = {
       key: value for key, value in inflow_var.items() if key in ["x", "y", "z"]
    }
    inflow_invar = normalize_invar(inflow_invar, center, 1./length_scale_amp, dims=3)
    inflow_outvar = {
       key: value*(1./vel_scale_amp) for key, value in inflow_var.items() if key in ["us_mag_noisy"]
    }
    doppler_numpy = PointwiseConstraint.from_numpy(
       nodes,
       inflow_invar,
    ...
    domain.add_constraint(doppler_numpy, "113_cut_down_US_MAG")
    
    inflow_var = csv_to_dict(
       to_absolute_path("./FSI_model/3_point_training_with_105/105/sampled_DOPPLER_SIGNAL_cut_down_interpolated_points_with_noise_threshold.csv"))
    inflow_invar = {
       key: value for key, value in inflow_var.items() if key in ["x", "y", "z"]
    }
    inflow_invar = normalize_invar(inflow_invar, center, 1./length_scale_amp, dims=3)
    inflow_outvar = {
       key: value*(1./vel_scale_amp) for key, value in inflow_var.items() if key in ["us_mag_noisy_0"]
    }
    doppler_numpy = PointwiseConstraint.from_numpy(
       nodes,
       inflow_invar,
    ...
    domain.add_constraint(doppler_numpy, "105_US_MAG")
    
   
    
    interior_pts=interior_mesh_pre.sample_interior(220000)
    ...
    nodes=nodes, invar=openfoam_invar_numpy, output_names=["u_0_scaled", "v_0_scaled", "w_0_scaled","p_0_scaled", "partial_ux_0_scaled", "partial_uy_0_scaled", "partial_uz_0_scaled", "partial_vx_0_scaled", "partial_vy_0_scaled", "partial_vz_0_scaled", "partial_wx_0_scaled", "partial_wy_0_scaled", "partial_wz_0_scaled"], requires_grad=True
    )
    domain.add_inferencer(openfoam_inferencer, "inf_data_validation_pre")
    
    interior_pts=interior_mesh_2nd_time.sample_interior(220000)
    ...
    openfoam_inferencer=PointwiseInferencer(
    nodes=nodes, invar=openfoam_invar_numpy, output_names=["u_1_scaled", "v_1_scaled", "w_1_scaled","p_1_scaled", "partial_ux_1_scaled", "partial_uy_1_scaled", "partial_uz_1_scaled", "partial_vx_1_scaled", "partial_vy_1_scaled", "partial_vz_1_scaled", "partial_wx_1_scaled", "partial_wy_1_scaled", "partial_wz_1_scaled"], requires_grad=True
    )
    domain.add_inferencer(openfoam_inferencer, "inf_data_validiation_post")
    
    
    interior_pts=interior_mesh_3rd_time.sample_interior(220000)
    ...
    openfoam_inferencer=PointwiseInferencer(
    nodes=nodes, invar=openfoam_invar_numpy, output_names=["u_scaled", "v_scaled", "w_scaled","p_scaled", "partial_ux_scaled", "partial_uy_scaled", "partial_uz_scaled", "partial_vx_scaled", "partial_vy_scaled", "partial_vz_scaled", "partial_wx_scaled", "partial_wy_scaled", "partial_wz_scaled"], requires_grad=True
    )
    domain.add_inferencer(openfoam_inferencer, "inf_data_validiation_post")
    
    
    inflow_var = csv_to_dict(
       to_absolute_path("./FSI_model/3_point_training_with_105/105/WALL_WITH_NORMALS.csv"))
    inflow_invar = {
       key: value for key, value in inflow_var.items() if key in ["x", "y", "z"]
    }
    inflow_invar = normalize_invar(inflow_invar, center, 1./length_scale_amp, dims=3)
    ...
    openfoam_inferencer=PointwiseInferencer(
    nodes=nodes, invar=inflow_invar, output_names=["u_0_scaled", "v_0_scaled", "w_0_scaled","p_0_scaled", "partial_ux_0_scaled", "partial_uy_0_scaled", "partial_uz_0_scaled", "partial_vx_0_scaled", "partial_vy_0_scaled", "partial_vz_0_scaled", "partial_wx_0_scaled", "partial_wy_0_scaled", "partial_wz_0_scaled"], requires_grad=True
    )
    domain.add_inferencer(openfoam_inferencer, "inf_data_validation_pre_sampled")
    
    # openfoam_invar_numpy={"x":interior_pts_pre["x"],
                          # "y":interior_pts_pre["y"],
                          # "z":interior_pts_pre["z"],
                          # }
    # openfoam_inferencer=PointwiseInferencer(
    # nodes=nodes, invar=openfoam_invar_numpy, output_names=["u_0_scaled", "v_0_scaled", "w_0_scaled", "p_0_scaled","u_scaled", "v_scaled", "w_scaled","p_scaled"]
    # )
