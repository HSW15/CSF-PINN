...
class TemporalNavierStokes_3rd(PDE):
    """
    ...
    def __init__(self, nu, rho=1, dim=3, time=True):
        ...
        u_0,v_0,w_0 = Symbol("u_2"), Symbol("v_2"), Symbol("w_2")
        ...
        # set equations
        self.equations = {}
        self.equations["continuity_3rd"] = (
            rho.diff(t) + (rho * u).diff(x) + (rho * v).diff(y) + (rho * w).diff(z)
        )
        self.equations["continuity_dx_3rd"] = (
            rho.diff(t) + (rho * u).diff(x) + (rho * v).diff(y) + (rho * w).diff(z)
        ).diff(x)
        self.equations["continuity_dy_3rd"] = (
            rho.diff(t) + (rho * u).diff(x) + (rho * v).diff(y) + (rho * w).diff(z)
        ).diff(y)
        self.equations["continuity_dz_3rd"] = (
            rho.diff(t) + (rho * u).diff(x) + (rho * v).diff(y) + (rho * w).diff(z)
        ).diff(z)
        self.equations["momentum_x_3rd"] = (
            (rho * u).diff(t)
            + rho * ((u - u_0) / (time_step))
            + (
                u * ((rho * u).diff(x))
        ...
        self.equations["momentum_x_dx_3rd"] = (
            (
                (rho * u).diff(t)
                + rho * ((u - u_0) / (time_step))
                + (
        ...
        self.equations["momentum_x_dy_3rd"] = (
            (
                (rho * u).diff(t)
                + rho * ((u - u_0) / (time_step))
                + (
        ...
        self.equations["momentum_x_dz_3rd"] = (
            (
                (rho * u).diff(t)
                + rho * ((u - u_0) / (time_step))
                + (
        ...
        self.equations["momentum_y_dx_3rd"] = (
            (
                (rho * v).diff(t)
                + rho * ((v - v_0) / (time_step))
                + (
        ...
        self.equations["momentum_y_dy_3rd"] = (
            (
                (rho * v).diff(t)
                + rho * ((v - v_0) / (time_step))
                + (
        ...
        self.equations["momentum_y_dz_3rd"] = (
            (
                (rho * v).diff(t)
                + rho * ((v - v_0) / (time_step))
                + (
        ...
        self.equations["momentum_z_dx_3rd"] = (
            (
                (rho * w).diff(t)
                + rho * ((w - w_0) / (time_step))
                + (
        ...
        self.equations["momentum_z_dy_3rd"] = (
            (
                (rho * w).diff(t)
                + rho * ((w - w_0) / (time_step))
                + (
        ...
        self.equations["momentum_z_dz_3rd"] = (
            (
                (rho * w).diff(t)
                + rho * ((w - w_0) / (time_step))
                + (
        ...
        if self.dim == 2:
            self.equations.pop("momentum_z_3rd")
            self.equations.pop("continuity_dz_3rd")
            self.equations.pop("momentum_x_dz_3rd")
            self.equations.pop("momentum_y_dz_3rd")
            self.equations.pop("momentum_z_dx_3rd")
            self.equations.pop("momentum_z_dy_3rd")
            self.equations.pop("momentum_z_dz_3rd")
