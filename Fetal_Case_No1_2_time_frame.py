...
from modulusDL.pdes.modified_temporal_navier_stokes import TemporalNavierStokes
from modulusDL.pdes.modified_temporal_navier_stokes_two_way import TemporalNavierStokes_two_way
...
def run(cfg: ModulusConfig) -> None:
    ...
    ns_1st = TemporalNavierStokes(nu=nd.ndim(nu), rho=nd.ndim(rho), dim=3, time=False)
    ns_2nd = TemporalNavierStokes_two_way(nu=nd.ndim(nu), rho=nd.ndim(rho), dim=3, time=False)
    ...
    vec_ref_mod_2nd_time=CustomModuleArch(
        input_keys=[Key("x"),Key("y"),Key("z")],
        output_keys=[Key("r_x"), Key("r_y"), Key("r_z")],
        module=transducer_probe(transducer_point)
    )
    
    US_doppler_2nd_time = CustomModuleArch(
        input_keys=[Key("r_x"), Key("r_y"), Key("r_z"),Key("u"), Key("v"), Key("w")],
    ...
    nodes = (
            ...
            + Scaler(
                ["u_0","v_0", "w_0", "p_0", "u","v", "w", "p"],
                ["u_0_scaled", "v_0_scaled", "w_0_scaled", "p_0_scaled", "u_scaled", "v_scaled", "w_scaled", "p_scaled"],
                ["m/s", "m/s","m/s", "m^2/s^2","m/s", "m/s","m/s", "m^2/s^2"],
                nd,
            ).make_node()
    )
    ...
    inflow_var = csv_to_dict(
       to_absolute_path("./temporal_stls/3_time_point_temporal/1082/new_visc_model/inlet_without_1.csv"))
    inflow_invar = {
    ...
    inflow_var = csv_to_dict(
       to_absolute_path("./temporal_stls/3_time_point_temporal/1072/new_visc_model/1072_inlet.csv"))
    inflow_invar = {
       key: value for key, value in inflow_var.items() if key in ["x", "y", "z"]
    }
    inflow_invar = normalize_invar(inflow_invar, center, 1./length_scale_amp, dims=3)
    ...
    inflow_var = csv_to_dict(
       to_absolute_path("./temporal_stls/3_time_point_temporal/1072/new_visc_model/1072_wall.csv"))
    inflow_invar = {
       key: value for key, value in inflow_var.items() if key in ["x", "y", "z"]
    }
    inflow_invar = normalize_invar(inflow_invar, center, 1./length_scale_amp, dims=3)
    ...
    inflow_var = csv_to_dict(
       to_absolute_path("./temporal_stls/3_time_point_temporal/1082/new_visc_model/wall.csv"))
    inflow_invar = {
       key: value for key, value in inflow_var.items() if key in ["x", "y", "z"]
    }
    inflow_invar = normalize_invar(inflow_invar, center, 1./length_scale_amp, dims=3)
    ...
    inflow_var = csv_to_dict(
       to_absolute_path("./temporal_stls/3_time_point_temporal/1072/new_visc_model/US_threshold_no_noise_new_model.csv"))
    inflow_invar = {
       key: value for key, value in inflow_var.items() if key in ["x", "y", "z"]
    }
    inflow_invar = normalize_invar(inflow_invar, center, 1./length_scale_amp, dims=3)
    ...
    inflow_var = csv_to_dict(
       to_absolute_path("./temporal_stls/3_time_point_temporal/1082/new_visc_model/US_threshold_1082_no_noise_without_1.csv"))
    inflow_invar = {
       key: value for key, value in inflow_var.items() if key in ["x", "y", "z"]
    }
    inflow_invar = normalize_invar(inflow_invar, center, 1./length_scale_amp, dims=3)
