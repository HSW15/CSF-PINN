...
def run(cfg: ModulusConfig) -> None:
    ...
    interior_mesh_3rd_time = Tessellation.from_stl(
        point_path + "/Interior/P2_wk31_interior_01112_interior.stl", airtight=True
    )
    ...
    flow_net_pre.load_state_dict(
        torch.load(
            "/examples/outputs/P2_wk31_temporal_redone_2dopplers_redone_geometry_3time_points_redone_two_way_couple_no_intersection_reduced_doppler_viscosity/flow_net_3rd_time.0.pth"
        ))
    for param in flow_net_pre.parameters():
        param.requires_grad = True
        flowfile_net="/examples/outputs/"+str(os.path.basename(__file__))[:-3]+"/flow_network_pre.0.pth"
    if not(os.path.isfile(flowfile_net)): #if file exist = true, dont run below. if file doesnt exist, run below
            ...
            shutil.copy("/examples/outputs/P2_wk31_temporal_redone_2dopplers_redone_geometry_3time_points_redone_two_way_couple_no_intersection_reduced_doppler_viscosity/flow_net_3rd_time.0.pth", flowfile_net)
    ...
    print("loading flow NN...")
    flow_net_2nd_time.load_state_dict(
        torch.load(
            "/examples/outputs/P2_wk31_temporal_redone_2dopplers_redone_geometry_3time_points_redone_two_way_couple_no_intersection_reduced_doppler_viscosity/flow_net_3rd_time.0.pth"
        ))
    for param in flow_net_2nd_time.parameters():
        param.requires_grad = True
        flowfile_net="/examples/outputs/"+str(os.path.basename(__file__))[:-3]+"/flow_net_4th_time.0.pth"
    if not(os.path.isfile(flowfile_net)): #if file exist = true, dont run below. if file doesnt exist, run below
            print("Copying file to new directory flow net pre")
            os.makedirs("/examples/outputs/"+str(os.path.basename(__file__))[:-3],exist_ok =True) #make directory of LV_full_model_part2
            shutil.copy("/examples/outputs/P2_wk31_temporal_redone_2dopplers_redone_geometry_3time_points_redone_two_way_couple_no_intersection_reduced_doppler_viscosity/flow_net_3rd_time.0.pth", flowfile_net)
    ...
    print("loading flow NN...")
    flow_net_3rd_time.load_state_dict(
        torch.load(
            "/examples/outputs/P2_wk31_temporal_redone_2dopplers_redone_geometry_3time_points_redone_two_way_couple_no_intersection_reduced_doppler_viscosity/flow_net_3rd_time.0.pth"
        ))
    for param in flow_net_3rd_time.parameters():
        param.requires_grad = True
        flowfile_net="/examples/outputs/"+str(os.path.basename(__file__))[:-3]+"/flow_net_5th_time.0.pth"
    if not(os.path.isfile(flowfile_net)): #if file exist = true, dont run below. if file doesnt exist, run below
            print("Copying file to new directory flow net pre")
            os.makedirs("/examples/outputs/"+str(os.path.basename(__file__))[:-3],exist_ok =True) #make directory of LV_full_model_part2
            shutil.copy("/examples/outputs/P2_wk31_temporal_redone_2dopplers_redone_geometry_3time_points_redone_two_way_couple_no_intersection_reduced_doppler_viscosity/flow_net_3rd_time.0.pth", flowfile_net)
    ...
    inflow_var = csv_to_dict(
       to_absolute_path("./temporal_stls/3_time_point_temporal/1112/new_visc_model/inlet_1112.csv"))
    inflow_invar = {
    ...
    domain.add_constraint(inlet_numpy, "inlet_numpy_1112")
    
    inflow_var = csv_to_dict(
       to_absolute_path("./temporal_stls/3_time_point_temporal/1102/new_visc_model/inlet_redone.csv"))
    inflow_invar = {
       key: value for key, value in inflow_var.items() if key in ["x", "y", "z"]
    }
    inflow_invar = normalize_invar(inflow_invar, center, 1./length_scale_amp, dims=3)
    ...
    domain.add_constraint(inlet_numpy, "inlet_numpy_1102")
    
    
    inflow_var = csv_to_dict(
       to_absolute_path("./temporal_stls/3_time_point_temporal/1092/new_visc_model/inlet.csv"))
    inflow_invar = {
       key: value for key, value in inflow_var.items() if key in ["x", "y", "z"]
    }
    inflow_invar = normalize_invar(inflow_invar, center, 1./length_scale_amp, dims=3)
    ...
    domain.add_constraint(inlet_numpy, "inlet_numpy_1092")
    
    
    inflow_var = csv_to_dict(
       to_absolute_path("./temporal_stls/3_time_point_temporal/1092/new_visc_model/wall.csv"))
    inflow_invar = {
       key: value for key, value in inflow_var.items() if key in ["x", "y", "z"]
    }
    inflow_invar = normalize_invar(inflow_invar, center, 1./length_scale_amp, dims=3)
    ...
    inflow_var = csv_to_dict(
       to_absolute_path("./temporal_stls/3_time_point_temporal/1102/new_visc_model/wall_redone.csv"))
    inflow_invar = {
       key: value for key, value in inflow_var.items() if key in ["x", "y", "z"]
    }
    inflow_invar = normalize_invar(inflow_invar, center, 1./length_scale_amp, dims=3)
    ...
    inflow_var = csv_to_dict(
       to_absolute_path("./temporal_stls/3_time_point_temporal/1112/new_visc_model/wall_1112.csv"))
    inflow_invar = {
       key: value for key, value in inflow_var.items() if key in ["x", "y", "z"]
    }
    inflow_invar = normalize_invar(inflow_invar, center, 1./length_scale_amp, dims=3)
    ...
    inflow_var = csv_to_dict(
       to_absolute_path("./temporal_stls/3_time_point_temporal/1092/new_visc_model/US_threshold_1092_no_noise.csv"))
    inflow_invar = {
       key: value for key, value in inflow_var.items() if key in ["x", "y", "z"]
    }
    inflow_invar = normalize_invar(inflow_invar, center, 1./length_scale_amp, dims=3)
    ...
    inflow_var = csv_to_dict(
       to_absolute_path("./temporal_stls/3_time_point_temporal/1102/new_visc_model/US_threshold_1102_no_noise_redone.csv"))
    inflow_invar = {
       key: value for key, value in inflow_var.items() if key in ["x", "y", "z"]
    }
    inflow_invar = normalize_invar(inflow_invar, center, 1./length_scale_amp, dims=3)
    ...
    inflow_var = csv_to_dict(
       to_absolute_path("./temporal_stls/3_time_point_temporal/1112/new_visc_model/US_doppler_threshold_1112.csv"))
    inflow_invar = {
       key: value for key, value in inflow_var.items() if key in ["x", "y", "z"]
    }
    inflow_invar = normalize_invar(inflow_invar, center, 1./length_scale_amp, dims=3)
