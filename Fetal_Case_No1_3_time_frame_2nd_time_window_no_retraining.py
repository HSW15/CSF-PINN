...
def run(cfg: ModulusConfig) -> None:
    ...
    flow_net_pre.load_state_dict(
        torch.load(
            "/examples/outputs/P2_wk31_temporal_redone_2dopplers_redone_geometry_3time_points_redone_two_way_couple_no_intersection/flow_net_3rd_time.0.pth"
        ))
    ...
    inflow_var = csv_to_dict(
       to_absolute_path("./temporal_stls/3_time_point_temporal/1102/inlet_1102.csv"))
    inflow_invar = {
       key: value for key, value in inflow_var.items() if key in ["x", "y", "z"]
    }
    inflow_invar = normalize_invar(inflow_invar, center, 1./length_scale_amp, dims=3)
    ...
    inflow_var = csv_to_dict(
       to_absolute_path("./temporal_stls/3_time_point_temporal/1102/US_DOPPLER_THRESHOLD_1102.csv"))
    inflow_invar = {
       key: value for key, value in inflow_var.items() if key in ["x", "y", "z"]
    }
    inflow_invar = normalize_invar(inflow_invar, center, 1./length_scale_amp, dims=3)
    ...
    inflow_var = csv_to_dict(
       to_absolute_path("./temporal_stls/3_time_point_temporal/1112/US_DOPPLER_THRESHOLD_1112.csv"))
    inflow_invar = {
       key: value for key, value in inflow_var.items() if key in ["x", "y", "z"]
    }
    inflow_invar = normalize_invar(inflow_invar, center, 1./length_scale_amp, dims=3)
